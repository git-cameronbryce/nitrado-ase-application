    ...
    client.on(Events.InteractionCreate, async interaction => {
      ...
      if (interaction.customId.startsWith('overwrite-admin-thread')) {
        ...
        const deletion = async (item) => {
          await db.collection('ase-configuration').doc(interaction.guild.id).update({ [`admin.${item[0]}`]: FieldValue.delete() })
            .then(async () => {
      ...
      if (interaction.customId.startsWith('overwrite-chat-thread')) {
        ...
        const deletion = async (item) => {
          await db.collection('ase-configuration').doc(interaction.guild.id).update({ [`chat.${item[0]}`]: FieldValue.delete() })
            .then(async () => {
              const thread = await interaction.client.channels.fetch(identifier);
              await thread.delete();
            });
